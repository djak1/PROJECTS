...
// LaplacianHW funkcija koja ce se reazlizovati u hardveru
//
...
//setLabel ispisuje ime u centru prepoznatog oblika
...
int main(int argc, char* argv[])
{
    ...
    char inFileName_Pin[128]="oblici.pgm";//ulazna slika
    ...
    if (LoadP5Header(infile_Pin, Pin)) // load pgm (Pin) image header information
    {
     ...
     cv::Mat src = cv::imread("/home/djak1188/Desktop/LEGOLAS/oblici.pgm"); 
        if (src.empty())
            return -1;
    ...
    for (int i = 0; i < contours.size(); i++)
    {
        ...
        if (std::fabs(cv::contourArea(contours[i])) > 100 && cv::isContourConvex(approx)) {
            ...
            if (approx.size() == 3)
            {
                setLabel(dst, "TROUGAO", contours[i]);    // Triangles
            }
            else if (approx.size() >= 4 && approx.size() <= 12)
            {
                ...
                if (vtc == 4 && mincos >= -0.15 && maxcos <= 0.15 &&
                    ...
                    setLabel(dst, "KVADRAT", contours[i]);
                }

                
                else if (vtc == 4 && mincos >= -0.15 && maxcos <= 0.15)
                    setLabel(dst, "PRAVOUGAONIK", contours[i]);
                    ...
                    setLabel(dst, "PETOUGAO", contours[i]);
                    ...
                    if (std::abs(1 - ((double)r.width / r.height)) <= 0.2 &&
                        ...
                        setLabel(dst, "KRUG", contours[i]);
                    }
                    
                   
                    else
                        setLabel(dst, "ELIPSA", contours[i]);
                }
    ...
    cv::imshow("Ulazna slika", src);
    cv::imshow("Konacni izlaz", dst);
    cv::waitKey(0);
